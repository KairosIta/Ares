...
# ---------------------------------------------------------------------------
# Git, letto senza eseguire git quando basta
# ---------------------------------------------------------------------------


def _directory_git(percorso: Path) -> Path | None:
...
def ramo_git(percorso: Path) -> str | None:
    ...
    Legge `HEAD` invece di lanciare `git`: il banner compare a ogni avvio e
    non deve aspettare un processo, ne' fallire dove git non e' installato.
    """
    git = _directory_git(percorso)
    if git is None:
        return None
    try:

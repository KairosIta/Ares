...
import config
from assistant_runtime import build_db
...
def costruisci_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Retention delle sessioni e dei risultati tool di Ares")
...
def _applica(user_id: str, sessioni: Sequence[SessioneRetention], yes: bool) -> int:
    ...
    print("Per tornare indietro:", python, "backup.py restore", snapshot.name)
    return 0

...
def _stampa_elenco(operazioni: OperazioniBackup) -> None:
    ...
    for percorso in reversed(snapshot):
        try:
            manifest = json.loads((percorso / backup_integrity.MANIFEST).read_text(encoding="utf-8"))
...
def _pota(args: argparse.Namespace, operazioni: OperazioniBackup) -> int:
    if args.keep < 1:
        raise backup_integrity.ErroreBackup("--keep deve essere almeno 1")
    ...
    with lock_stato(esclusivo=True):
        ...
        if [percorso.name for percorso in candidati_attuali] != nomi_visti:
            raise backup_integrity.ErroreBackup("l'elenco degli snapshot e' cambiato; ripeti prune")
...
def main(operazioni: OperazioniBackup, argomenti: Sequence[str] | None = None) -> int:
    ...
    except (backup_integrity.ErroreBackup, StatoOccupato, OSError) as errore:
        print("ERRORE:", errore)
        return 1
    return 0

...
def _prepara_restore(snapshot: Path, manifest: dict[str, Any]) -> Path:
    ...
    try:
        for nome in backup_integrity.DATABASE:
            if componenti.get(nome):
                shutil.copy2(snapshot / nome, staging / nome)
                backup_integrity.verifica_sqlite(staging / nome)
        ...
        if lance.get("present"):
            ...
            if backup_integrity.conta_tabelle_lancedb(staging / "lancedb") != lance.get("tables"):
                raise backup_integrity.ErroreBackup("LanceDB cambia durante la preparazione del restore")
        _privato(staging)
        return staging
    except Exception:
...
def _installa_restore_per_copia(staging: Path, destinazione: Path, precedente: Path) -> None:
    ...
    try:
        ...
        try:
            ...
            raise backup_integrity.ErroreBackup(
                "restore fallito ("
